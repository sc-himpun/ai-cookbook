...
def describe_entity_set(entity_set: str, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    entity_type = next((es.attrib.get("EntityType").split('.')[-1] for es in root.findall(".//edm:EntitySet", ns) if es.attrib.get("Name") == entity_set), None)
    if not entity_type:
        return f"Entity set '{entity_set}' not found."
    fields = [f"{p.attrib.get('Name')}: {p.attrib.get('Type')}" for et in root.findall(f".//edm:EntityType[@Name='{entity_type}']", ns) for p in et.findall("edm:Property", ns)]
...
def apply_groupby_aggregate(entity_set: str, groupby: str, aggregate: str, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    try:
        ...
        for row in data:
            group_map[row.get(groupby)].append(row.get(aggregate.split()[0]))

        result = []
        for k, vals in group_map.items():
            try:
                agg_val = sum(vals) / len(vals) if "average" in aggregate else sum(vals)
                result.append({groupby: k, aggregate: agg_val})
            except Exception:
                result.append({groupby: k, aggregate: "❌ Calculation error"})
