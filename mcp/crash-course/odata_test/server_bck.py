...
def truncate_response(data: Any) -> str:
    ...
    return stringified[:MAX_CHARS] + "\n...(truncated)" if len(stringified) > MAX_CHARS else stringified
...
def search_entity(
    ...
    """Search a generic OData service with optional filter (e.g., Products, Orders). For northwind dataset, base_url is https://services.odata.org/V4/Northwind/Northwind.svc"""
    ...
    return "\n---\n".join(str(item) for item in results[:top])
...
def odata_expand_or_walk(
    ...
    Example: 
    - Walk: Orders(10643)?$expand=Employee($expand=ReportsTo)
    - Expand: Orders?$top=3&$expand=Employee
    """
    if "services.odata.org" not in base_url and email not in user_tokens:
        return "❌ Email not authorized for private OData service."
    ...
    if key:
        # Walk a navigation chain on a specific record
        url = f"{base_url.rstrip('/')}/{entity_set}({key})"
        params = {"$expand": expand_clause, "$format": "json"}
    else:
        # Bulk expand navigation on multiple records
        url = f"{base_url.rstrip('/')}/{entity_set}"
...
def odata_get_navigation_paths(
    ...
    def extract_paths(entity_type, prefix=""):
            ...
            if et.attrib.get("Name") == entity_type:
                for nav in et.findall("edm:NavigationProperty", ns):
                    ...
                    if to_type != entity_type:  # avoid infinite loop
                        extract_paths(to_type, prefix=full_path + "/")
