...
def get_github_creds(metadata: Optional[Dict]) -> Optional[str]:
    """
    Extract and optionally refresh GitHub access token from metadata.

    Args:
        metadata (Optional[Dict]): Dict with nested 'github' key.

    Returns:
        str (access_token) or None
    """
    if not metadata:
        return None
...
def get_github_client(metadata: Dict[str, Any]) -> Github:
    access_token = get_github_creds(metadata)
    if not access_token:
        raise Exception("❌ Missing or invalid GitHub access token in metadata.")
...
def github_list_pull_requests(metadata: Dict[str, Any], repo_name: str, state: str = "open") -> str:
...
def github_create_branch(metadata: Dict[str, Any], repo_name: str, new_branch: str, base_branch: str) -> str:
...
def github_list_files_in_path(metadata: Dict[str, Any], repo_name: str, path: str = "", ref: str = "main") -> str:
...
def github_list_folders_in_path(metadata: Dict[str, Any], repo_name: str, path: str = "", ref: str = "main") -> str:
...
def github_get_file_content(metadata: Dict[str, Any], repo_name: str, file_path: str, ref: str = "main") -> str:
...
def github_search_files(metadata: Dict[str, Any], repo_name: str, query: str, ref: str = "main") -> str:
...
def github_delete_branch(metadata: Dict[str, Any], repo_name: str, branch_name: str) -> str:
...
def github_create_pull_request(metadata: Dict[str, Any], repo_name: str, source_branch: str, target_branch: str, title: str, body: str = "") -> str:
...
def github_close_pull_request(metadata: Dict[str, Any], repo_name: str, pr_number: int) -> str:
...
def github_create_commit(metadata: Dict[str, Any], repo_name: str, branch: str, file_path: str, message: str, content: str) -> str:
...
def github_merge_pull_request(metadata: Dict[str, Any], repo_name: str, pr_number: int, merge_message: str = "") -> str:
...
def github_search_file_contents(metadata: Dict[str, Any], repo_name: str, keyword: str, ref: str = "main") -> str:
...
def github_list_closed_issues(metadata: Dict[str, Any], repo_name: str) -> str:
...
def github_issue_counts(metadata: Dict[str, Any], repo_name: str) -> str:
...
def github_get_authorization_url(metadata: Dict[str, Any]) -> str:
...
def github_list_authorized_accounts(metadata: Dict[str, Any]) -> str:
    """List all GitHub email accounts that have been authenticated."""
    return metadata.get("email") if metadata.get("email") else "No accounts authorized."
...
async def oauth2callback(request: Request):
    ...
    return JSONResponse({
        "message": f"Authenticated as {primary_email}",
        "email": primary_email,
