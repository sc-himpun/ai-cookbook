...
def make_response(success: bool, action: str, message: str, data: Any = None) -> Dict[str, Any]:
    """
...
def search_files(
    ...
    try:
        ...
        for obj in sorted(all_objects, key=lambda x: x["Size"]):
            ...
            if file_size > LARGE_FILE_THRESHOLD:
                threshold_dict = make_large_file_entry(s3, bucket, key, file_size)
                results.append(threshold_dict)
                message += f"{threshold_dict['key']} content not searched due to {threshold_dict['message']} ,"
                continue
            ...
            if match_info:
                ...
                message += f"{match_info['key']} matched,"

        return make_response(
            success=True,
            action=action,
            message=f"Found {len(results)} matching files in bucket '{bucket}', Details: {message}",
            data=results,
        )
...
def make_large_file_entry(s3, bucket: str, key: str, size: int) -> dict:
    ...
    """
    msg = f"""File size {size} exceeds threshold. File is a possible candidate for search but its content not yet searched.
    Use tool `vector_ingest_s3` for vector search if needed (might need long time).
    Prompt user for confirmation to search on this file using vector search approach.
    """
    return {
        "key": key,
        "size": size,
...
def search_pdf(s3, bucket: str, key: str, keyword: str) -> tuple[bool, list[int], int]:
    """
...
def search_docx(s3, bucket: str, key: str, keyword: str) -> tuple[bool, list[int], int]:
    """
...
def search_text(s3, bucket: str, key: str, keyword: str, max_preview_chars: int) -> tuple[bool, list[int], int]:
    """
